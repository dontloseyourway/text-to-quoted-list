...
- 0.3.9：修复 devops 等页面点击空白处无法隐藏面板（页面阻止了事件冒泡）。
- 0.3.8：修复点击 Logo 面板闪现、拖拽 Logo 时面板行为异常的问题。
- 0.3.7：修复 Logo 点击无法切换面板、部分页面空白处点击无法隐藏面板的问题。
- 0.3.6：修复部分页面出现两个悬浮 Logo（frames 注入/重复挂载导致）。
